...
static int registerlocalvar (LexState *ls, TString *varname) {
  ...
  // 因为FuncState只是编译器的中间产物，它需要维护额外的数据来分析脚本、生成Proto
  // Proto最终会装入Closure压栈，传递给解释器执行，那个时候FuncState就没有什么作用了
...
// 分析一个lua源代码文件的主函数
// ZIO *z 输入流对象，内含文件读取缓冲区（从属于SParser）
// MBuffer *buff 解析缓冲区（从属于SParser）
// const char *name 文件名
Proto *luaY_parser (lua_State *L, ZIO *z, Mbuffer *buff, const char *name) {
  struct LexState lexstate;			//词法分析（分词）器
  ...
  open_func(&lexstate, &funcstate);							// 初始化分词器的funcstate部分
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  int pc = luaK_codeABC(fs, OP_NEWTABLE, 0, 0, 0);		// 变量在栈上的位置、数组size、散列表size
  struct ConsControl cc;
  cc.na = cc.nh = cc.tostore = 0;
  cc.t = t;
  init_exp(t, VRELOCABLE, pc);
  init_exp(&cc.v, VVOID, 0);  /* no value (yet) */
  luaK_exp2nextreg(ls->fs, t);  /* fix it at stack top (for gc) 调整第一个参数（table存入栈的位置） */
  checknext(ls, '{');
  do {
    ...
    switch(ls->t.token) {
      case TK_NAME: {  /* may be listfields or recfields */
        ...
        if (ls->lookahead.token != '=')  /* expression? */
          listfield(ls, &cc);				// 成员以数组方式存入
        else
          recfield(ls, &cc);				// 成员以散列表方式存入
        break;
      }
